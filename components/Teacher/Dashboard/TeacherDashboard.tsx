...
import StudentResgister from './StudentResgister';

 const DashViewData = [
  { title: "New Students", counts: "14", img: studyMatIcon },
  { title: "Total Students", counts: "20", img: trainNotIcon },
  { title: "Toatal Income", counts: "30", img: earningIcon },
  { title: "Total woriking hours", counts: "40", img: totalIcon },
    ...
    <>
      <div className=" mx-auto">
        <div className="flex flex-col lg:flex-row justify-between  items-center gap-4 lg:gap-0">
          {/* Student Rating - Full width on mobile, auto width on desktop */}
          <div className="w-full lg:w-auto">
            <StudentRating />
          </div>

          {/* Dashboard Cards Grid */}
          <div className="w-full lg:w-[600px] grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-5  md:h-[250px]">
            {DashViewData.map((item, index) => (
              <div
                key={index}
                className="p-2 md:p-3 textColor rounded-[10px] cursor-pointer bg-white border-2 border-logoPrimary shadow-lg"
              >
                <div className="flex gap-2 md:gap-4">
                  <Image className="w-6 h-6 md:w-8 md:h-8" src={item.img} alt="loading..." />
                  <div className="flex flex-col">
                    <h1 className="text-xs md:text-sm text-textColor">{item.title}</h1>
                  </div>
                </div>
                <div className="flex justify-around items-center mt-2 md:mt-0">
                  <h1 className="text-xl md:text-2xl font-medium text-customBlue">
                    {item.counts}
                  </h1>
                  <svg
