...
function StudentRegistration({ onclose }: { onclose?: () => void }) {
  ...
  const studentType = user_types?.find((item: any) => item.name === "student");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (studentType?.id) {
        submitData.append("type", studentType.id.toString());
      }
      ...
      submitData.append('referralCode', formData.referral_code);
      if (formData.profile_image) {
        submitData.append('profile_image', formData.profile_image);
      }

      // Make API call here
      const response: any = await UserRegistrationApiService.studentRegister(submitData);
      ...
      <div className='grid xl:grid-cols-2 gap-4'>
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            onChange={(e) =>
              setFormData((prev) => ({ ...prev, referral_code: e.target.value }))
            }
