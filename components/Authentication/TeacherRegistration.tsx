...
function TeacherRegistration({ onclose }: { onclose?: () => void }) {
  ...
  const teacherType = user_types?.find((item: any) => item.name === "teacher");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (teacherType?.id) {
        submitData.append("type", teacherType.id.toString());
      }
      ...
      // Make API call here
      const response: any = await UserRegistrationApiService.teacherRegister(submitData);
